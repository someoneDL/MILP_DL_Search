...
def var_x(x, r):
    '''x variable'''
    return [[x + "_" + str(r) + "_" + str(i) + "_" + str(j) for j in range(2)] for i in range(16)]

def var_y(y, r):
    '''y variable'''
    return [[y + "_" + str(r) + "_" + str(i) + "_" + str(j) for j in range(4)] for i in range(16)]

def var_p(r):
    return [["p_" + str(r) + "_" + str(i) + "_" + str(j) for j in range(2)] for i in range(16)]

def flat(state, num):
    """Flatten the list of lists 'state' into a single list."""
    a = []
    for i in range(num):
        a += state[i]
    return a
...
def Model_I(s_in, f):
    ...
    for a in k:
        ...
        a = a.strip('[')
        a = a.strip(']')
        a = a.split(',')  # Split the string: Slice the string by specifying the delimiter and return the split string list
        MLI_coe.append(a)
    fl.close()
...
def Model_MDS(s_in, s_mi, s_out, f):
    ...
    # s_in,s_mi,s_out
    # s_in1=[s_in1['x'][i],s_in1['y'][i],s_in1['z'][i]]
    for i in range(4):
        c_in0 = {'x': s_in['x'][4 * i + 1], 'y': s_in['y'][4 * i + 1]}
        c_in1 = {'x': s_in['x'][4 * i + 2], 'y': s_in['y'][4 * i + 2]}
        c_in2 = {'x': s_in['x'][4 * i + 3], 'y': s_in['y'][4 * i + 3]}
        c_m = {'x': s_mi['x'][4 * i], 'y': s_mi['y'][4 * i]}
        ...
        c_in1 = {'x': s_in['x'][4 * i + 2], 'y': s_in['y'][4 * i + 2]}
        c_in2 = {'x': s_in['x'][4 * i + 3], 'y': s_in['y'][4 * i + 3]}
        c_m = {'x': s_mi['x'][4 * i + 1], 'y': s_mi['y'][4 * i + 1]}
        c_out = {'x': s_out['x'][4 * i + 1], 'y': s_out['y'][4 * i + 1]}
        Model_XOR(c_in0, c_in1, c_m, xor_coe, f)
        Model_XOR(c_m, c_in2, c_out, xor_coe, f)

        c_in0 = {'x': s_in['x'][4 * i], 'y': s_in['y'][4 * i]}
        c_in1 = {'x': s_in['x'][4 * i + 1], 'y': s_in['y'][4 * i + 1]}
        c_in2 = {'x': s_in['x'][4 * i + 3], 'y': s_in['y'][4 * i + 3]}
        c_m = {'x': s_mi['x'][4 * i + 2], 'y': s_mi['y'][4 * i + 2]}
        c_out = {'x': s_out['x'][4 * i + 2], 'y': s_out['y'][4 * i + 2]}
        Model_XOR(c_in0, c_in1, c_m, xor_coe, f)
        Model_XOR(c_m, c_in2, c_out, xor_coe, f)

        c_in0 = {'x': s_in['x'][4 * i], 'y': s_in['y'][4 * i]}
        c_in1 = {'x': s_in['x'][4 * i + 1], 'y': s_in['y'][4 * i + 1]}
        c_in2 = {'x': s_in['x'][4 * i + 2], 'y': s_in['y'][4 * i + 2]}
        c_m = {'x': s_mi['x'][4 * i + 3], 'y': s_mi['y'][4 * i + 3]}
        c_out = {'x': s_out['x'][4 * i + 3], 'y': s_out['y'][4 * i + 3]}
        Model_XOR(c_in0, c_in1, c_m, xor_coe, f)
        Model_XOR(c_m, c_in2, c_out, xor_coe, f)
    return 0

def shift(state):
    Ph = [0, 10, 5, 15, 14, 4, 11, 1, 9, 3, 12, 6, 7, 13, 2, 8]
    tmp = {'x': [], 'y': []}
    for i in range(16):
        tmp['x'].append(state['x'][Ph[i]])
        tmp['y'].append(state['y'][Ph[i]])
    return tmp
...
def product_lp(var_set, name, R, z_in=[], z_out = 16, d_in=[], c_out=[], flag=0,AP_a=[],AP_b=[]):
    ...
    s_in = {'x': var_x('x', 0), 'y': var_y('y', 0)}
    Model_I(s_in, f)
    var_set.extend(flat(s_in['x'], 16))
    var_set.extend(flat(s_in['y'], 16))

    # Initial input conditions: exist one N, the others are Z
    obj = ''
    for i in range(16):
        obj += str(s_in['x'][i][0]) + " + "
    obj = obj[:-3]
    ...
    if d_in != []:
        # The value corresponding to the input active word is fixed
        for z in z_in:
            for i in range(4):
                f.write(s_in['y'][z][i] + " = " + str(d_in[i]) + "\n")

    for r in range(R):
        s_m = {'x': var_x('xm', r), 'y': var_y('ym', r)}
        Model_I(s_m, f)
        var_set.extend(flat(s_m['x'], 16))
        var_set.extend(flat(s_m['y'], 16))

        s_mi = {'x': var_x('xmi', r), 'y': var_y('ymi', r)}
        Model_I(s_mi, f)
        var_set.extend(flat(s_mi['x'], 16))
        var_set.extend(flat(s_mi['y'], 16))

        s_out = {'x': var_x('x', r + 1), 'y': var_y('y', r + 1)}
        Model_I(s_out, f)
    ...
    # Additional conditions, triggered when AP!=[], fixed pattern path
    if AP_a != []:
        liss = ''
...
def Research_first(name, R):
    ...
    z_in =[]  # record the index of input active words
    W = []  # the position of output active word
    var_set = []
    product_lp(var_set, name, R)
    m = read(name)
    m.Params.Method = 3  # Set up the solution method
    m.Params.PoolSearchMode = 2
    m.optimize()
    if m.status == 2 or m.status == 11:
        m.write("result_look_first.sol")
    AP_a = []  # Record the active word in the middle
    AP_b = []  # Record the active word in the middle that going through S-box
    for r in range(R):
        # 0--(R-1)
        for i in range(16):
            nx = 'x_' + str(r) + '_' + str(i) + '_1'
            nxm = 'xm_' + str(r) + '_' + str(i) + '_1'  # Find active words (N) in each round
            if abs(m.getVarByName(name=nx).X - 1) < 0.1:
                AP_a.append(nx)
            if abs(m.getVarByName(name=nxm).X - 1) < 0.1:
                AP_b.append(nxm)
    print('-------------print trails -----\n',AP_a,AP_b)
    # Find a position, in order to locate active words, and the optimal target value
    aim = m.ObjVal
    for i in range(16):
        nx = 'x_0_' + str(i) + '_1'
        if abs(m.getVarByName(name=nx).X - 1) <0.1:
...
def operation(name, R, z_in, W, d_in, c_out, aim,AP_a,AP_b):
    ...
    var_set = []
    g = 10
    for i in range(aim, aim + g + 1):
        print(i, aim)
        product_lp(var_set, name, R, z_in, W, d_in, c_out, i, AP_a,AP_b)
        m = read(name)
        m.Params.Method = 3
        m.Params.PoolSearchMode = 2
        m.Params.PoolSolutions = 5**9
...
def M_Truncated_differntial(name, R):
    ...
    MASK = {'1': [8, 4, 0, 0, 0, 0, -4, -4, 0, 0, -4, -4, 4, 0, 0, 0],
            ...
            '15': [8, -4, 0, 0, -4, 0, 0, 0, 0, 0, 4, -4, 0, 0, -4,4]}  # The value of the output mask starts from 1, and the mask cannot be 0. key is the output mask, and value is the dlct corresponding to the input difference.
    PR = []
    ...
    for c in range(16):
        ...
        pr = operation(name, R, z_in, W, d_in, d_out, aim, AP_a,AP_b)  # Returns the truncated differential probability
        lis = [hex(c), pr]
        PR.append(lis)
    print(PR)
    ...
    for key in MASK.keys():
        bia = 0
        for c in range(16):
            if MASK[key][c] != 0:  # If the current difference-mask value is not equal to 0, then calculate the truncated difference probability
